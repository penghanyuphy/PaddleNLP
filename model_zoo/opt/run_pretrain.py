...
def do_train(args):
    ...
    model = OPTForCausalLM.from_pretrained(args.model_name_or_path,load_state_as_np=True)
    ...
    quanter = paddleslim.dygraph.quant.QAT(config=quant_configs)
        ...
        for f_id in range(num_files):
            ...
            for step, batch in enumerate(train_data_loader()):
                ...
                with paddle.amp.auto_cast(
                    ...
                    preds = model(tokens, position_ids, attention_mask)
                    labels_num = np.load("label_num.npy")
                    label_num_tensor = paddle.to_tensor(labels_num)
                    preds = preds - 0.1 * (label_num_tensor[:preds.shape[2]] + 1e-3).log()
                    loss = criterion(preds, labels, loss_mask)
