...
from modeling import OPTForCausalLM,OPTModel
...
# yapf: enable
parser.add_argument("--max_layer", default=24, type=int, help="Batch size per GPU/CPU for training.")
...
def do_eval(args):
    ...
    model = load(model,'/root/.paddlenlp/models/{}/model_state.pdparams'.format(args.model_name), max_layer=args.max_layer)
    model.eval()
    '''
    ...
    paddle.jit.save(static_model, '/root/.paddlenlp/models/facebook/opt-1.3b/model_state', input_spec=input_spec)
    '''
    ...
    with paddle.no_grad():
        for step, batch in enumerate(eval_data_loader):
            ...
            data = [(tokens, position_ids),attention_mask]
            preds = model.eval_batch(data)
            preds = preds[0]
            #print(preds[0,0,:100])
            #abc
